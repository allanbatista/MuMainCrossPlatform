const MARS_KEY_WORDS: usize = 40;
const MARS_SBOX_WORDS: usize = 512;
const MARS_SCRATCH_WORDS: usize = 15;

pub(crate) struct MarsCipher {
    l_key: [u32; MARS_KEY_WORDS],
}

impl MarsCipher {
    pub(crate) fn new(key: &[u8]) -> Self {
        let mut cipher = Self {
            l_key: [0u32; MARS_KEY_WORDS],
        };
    ...
    pub(crate) fn decrypt_block(&self, block: &mut [u8]) {
        ...
        let mut d = u32::from_le_bytes(block[0..4].try_into().unwrap()).wrapping_add(k[36]);
        let mut c = u32::from_le_bytes(block[4..8].try_into().unwrap()).wrapping_add(k[37]);
        let mut b = u32::from_le_bytes(block[8..12].try_into().unwrap()).wrapping_add(k[38]);
        let mut a = u32::from_le_bytes(block[12..16].try_into().unwrap()).wrapping_add(k[39]);

        let (na, nb, nc, nd) = mars_fmix(a, b, c, d, s);
        a = na.wrapping_add(nd);
        b = nb;
        c = nc;
        d = nd;

        let (na, nb, nc, nd) = mars_fmix(b, c, d, a, s);
        b = na.wrapping_add(nb);
        c = nb;
        d = nc;
        a = nd;

        let (na, nb, nc, nd) = mars_fmix(c, d, a, b, s);
        c = na;
        d = nb;
        a = nc;
        b = nd;

        let (na, nb, nc, nd) = mars_fmix(d, a, b, c, s);
        d = na;
        a = nb;
        b = nc;
        c = nd;
        ...
        let (na, nb, nc, nd) = mars_rktr(a, b, c, d, k, s, 34);
        a = na;
        b = nb;
        c = nc;
        d = nd;

        let (na, nb, nc, nd) = mars_rktr(b, c, d, a, k, s, 32);
        b = na;
        c = nb;
        d = nc;
        a = nd;

        let (na, nb, nc, nd) = mars_rktr(c, d, a, b, k, s, 30);
        c = na;
        d = nb;
        a = nc;
        b = nd;

        let (na, nb, nc, nd) = mars_rktr(d, a, b, c, k, s, 28);
        d = na;
        a = nb;
        b = nc;
        c = nd;

        let (na, nb, nc, nd) = mars_rktr(a, b, c, d, k, s, 26);
        a = na;
        b = nb;
        c = nc;
        d = nd;

        let (na, nb, nc, nd) = mars_rktr(b, c, d, a, k, s, 24);
        b = na;
        c = nb;
        d = nc;
        a = nd;

        let (na, nb, nc, nd) = mars_rktr(c, d, a, b, k, s, 22);
        c = na;
        d = nb;
        a = nc;
        b = nd;

        let (na, nb, nc, nd) = mars_rktr(d, a, b, c, k, s, 20);
        d = na;
        a = nb;
        b = nc;
        c = nd;

        let (na, nb, nc, nd) = mars_rktr(a, d, c, b, k, s, 18);
        a = na;
        d = nb;
        c = nc;
        b = nd;

        let (na, nb, nc, nd) = mars_rktr(b, a, d, c, k, s, 16);
        b = na;
        a = nb;
        d = nc;
        c = nd;

        let (na, nb, nc, nd) = mars_rktr(c, b, a, d, k, s, 14);
        c = na;
        b = nb;
        a = nc;
        d = nd;

        let (na, nb, nc, nd) = mars_rktr(d, c, b, a, k, s, 12);
        d = na;
        c = nb;
        b = nc;
        a = nd;

        let (na, nb, nc, nd) = mars_rktr(a, d, c, b, k, s, 10);
        a = na;
        d = nb;
        c = nc;
        b = nd;

        let (na, nb, nc, nd) = mars_rktr(b, a, d, c, k, s, 8);
        b = na;
        a = nb;
        d = nc;
        c = nd;

        let (na, nb, nc, nd) = mars_rktr(c, b, a, d, k, s, 6);
        c = na;
        b = nb;
        a = nc;
        d = nd;

        let (na, nb, nc, nd) = mars_rktr(d, c, b, a, k, s, 4);
        d = na;
        c = nb;
        b = nc;
        a = nd;

        let (na, nb, nc, nd) = mars_bmix(a, b, c, d, s);
        a = na;
        b = nb;
        c = nc;
        d = nd;

        let (na, nb, nc, nd) = mars_bmix(b, c, d, a, s);
        b = na;
        c = nb;
        d = nc;
        a = nd;
        ...
        let (na, nb, nc, nd) = mars_bmix(c, d, a, b, s);
        c = na;
        d = nb;
        a = nc;
        b = nd;
        ...
        let (na, nb, nc, nd) = mars_bmix(d, a, b, c, s);
        d = na;
        a = nb;
        b = nc;
        c = nd;
        ...
        let out_d = d.wrapping_sub(k[0]);
        let out_c = c.wrapping_sub(k[1]);
        let out_b = b.wrapping_sub(k[2]);
        let out_a = a.wrapping_sub(k[3]);

        block[0..4].copy_from_slice(&out_d.to_le_bytes());
        block[4..8].copy_from_slice(&out_c.to_le_bytes());
        block[8..12].copy_from_slice(&out_b.to_le_bytes());
        block[12..16].copy_from_slice(&out_a.to_le_bytes());
    }

    fn set_key(&mut self, in_key: &[u8]) {
        let s = &MARS_SBOX;
        let mut t = [0u32; MARS_SCRATCH_WORDS];

        for index in 0..(in_key.len() / 4) {
            t[index] = u32::from_le_bytes(in_key[index * 4..index * 4 + 4].try_into().unwrap());
        }
        t[in_key.len() / 4] = (in_key.len() / 4) as u32;

        for j in 0..4 {
            for i in 0..MARS_SCRATCH_WORDS {
                t[i] ^= (t[(i + 8) % MARS_SCRATCH_WORDS] ^ t[(i + 13) % MARS_SCRATCH_WORDS])
                    .rotate_left(3)
                    ^ ((4 * i + j) as u32);
            }
            ...
            for _ in 0..4 {
                for i in 0..MARS_SCRATCH_WORDS {
                    t[i] = t[i]
                        .wrapping_add(s[t[(i + 14) % MARS_SCRATCH_WORDS] as usize % MARS_SBOX_WORDS])
                        .rotate_left(9);
                }
            }

            for i in 0..10 {
                self.l_key[10 * j + i] = t[(4 * i) % MARS_SCRATCH_WORDS];
            }
        }

        for i in (5..37).step_by(2) {
            let mut w = self.l_key[i] | 3;
            let m = mars_gen_mask(w);
            if m != 0 {
                w ^= s[265 + (self.l_key[i] & 3) as usize].rotate_left(self.l_key[i - 1] & 31) & m;
            }
            self.l_key[i] = w;
        }
    }
...
fn mars_gen_mask(x: u32) -> u32 {
    let mut m = ((!x) ^ (x >> 1)) & 0x7FFF_FFFF;
    m &= m >> 1;
    m &= m >> 2;
    m &= m >> 3;
    m &= m >> 6;
...
fn mars_fmix(a: u32, b: u32, c: u32, d: u32, s: &[u32; MARS_SBOX_WORDS]) -> (u32, u32, u32, u32) {
    let mut r = a.rotate_right(8);
    let b = b ^ s[(a & 255) as usize];
    let b = b.wrapping_add(s[(r & 255) as usize + 256]);
    r = a.rotate_right(16);
    let a = a.rotate_right(24);
    let c = c.wrapping_add(s[(r & 255) as usize]);
    let d = d ^ s[(a & 255) as usize + 256];
    (a, b, c, d)
}

fn mars_bmix(a: u32, b: u32, c: u32, d: u32, s: &[u32; MARS_SBOX_WORDS]) -> (u32, u32, u32, u32) {
    let mut r = a.rotate_left(8);
    let b = b ^ s[(a & 255) as usize + 256];
    let c = c.wrapping_sub(s[(r & 255) as usize]);
    r = a.rotate_left(16);
    let a = a.rotate_left(24);
    let d = d.wrapping_sub(s[(r & 255) as usize + 256]);
    let d = d ^ s[(a & 255) as usize];
    (a, b, c, d)
}
...
fn mars_rktr(
    ...
    c: u32,
    d: u32,
    k: &[u32; MARS_KEY_WORDS],
    s: &[u32; MARS_SBOX_WORDS],
    index: usize,
) -> (u32, u32, u32, u32) {
    let mut r = a.wrapping_mul(k[index + 1]);
    let a = a.rotate_right(13);
    let m = a.wrapping_add(k[index]);
    let l0 = s[(m & 511) as usize];
    r = r.rotate_left(5);
    let mut l = l0 ^ r;
    let c = c.wrapping_sub(m.rotate_left(r & 31));
    r = r.rotate_left(5);
    l ^= r;
    let d = d ^ r;
    let b = b.wrapping_sub(l.rotate_left(r & 31));
    (a, b, c, d)
}

const MARS_SBOX: [u32; MARS_SBOX_WORDS] = [
...
#[cfg(test)]
mod tests {
    use super::MarsCipher;

    #[test]
    fn decrypts_known_vector() {
        let key = [0u8; 16];
        let mut block = [
            0xDC, 0xC0, 0x7B, 0x8D, 0xFB, 0x07, 0x38, 0xD6, 0xE3, 0x0A, 0x22, 0xDF, 0xCF, 0x27,
            0xE8, 0x86,
        ];
